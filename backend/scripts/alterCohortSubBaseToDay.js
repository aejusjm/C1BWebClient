// 기수 구독기준 시작/종료일 컬럼을 DATE → INT(일)로 변경
// 실행: node scripts/alterCohortSubBaseToDay.js
...
async function alterColumns() {
  try {
    ...
    if (dataType === 'date') {
      ...
      `);
      console.log('✅ tb_cohort.sub_base_start/end INT(일) 전환 완료');
    } else if (dataType === 'int') {
      console.log('ℹ️ 이미 INT 타입입니다. 변경 없음');
    } else {
      console.log(`ℹ️ sub_base_start 컬럼 상태: ${dataType || '없음'}`);
    }
  } catch (error) {
